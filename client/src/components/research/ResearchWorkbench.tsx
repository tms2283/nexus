/**
 * ResearchWorkbench — full research UI, responsive for mobile + desktop.
 *
 * Tabs:
 *   • "Notebook"      → Sources | Chat | Studio (stacked on mobile w/ sub-nav)
 *   • "Knowledge Tree"→ D3 force-graph + wiki (stacked on mobile)
 *   • "Charts"        → Heat-map / bar / bubble visualizations
 *   • "Rabbit Holes"  → Curated deep-dives with AI expansion
 */
...
const RABBIT_HOLES = [
  ...
  },
  {
    ...
    hook: "Abandoned by both Britain and China, Kowloon became a self-governing micro-nation with its own dentists, restaurants, social norms, and criminal underworld.",
  },
  {
    id: "sleep-paralysis",
    emoji: "😴",
    ...
    hook: "The 'Old Hag' of Newfoundland. The 'Kanashibari' of Japan. The 'Popobawa' of Zanzibar. Different continents, different centuries, identical experiences.",
  },
] as const;
...
function SourcesPanel({
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-[rgba(9,11,20,0.92)]">
      <div className="border-b border-white/10 p-4 flex-none">
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {isLoading ? (
          ...
          sources.map((s) => (
            <button key={s.id} onClick={() => onSelectSource(s.id)}
...
function ChatPanel({
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <div className="border-b border-white/10 px-4 py-3 flex-none">
      ...
      <div className="border-t border-white/10 p-3 flex-none">
        <div className="flex gap-2">
          <textarea value={input} onChange={(e) => setInput(e.target.value)}
            ...
            placeholder="Ask anything about your sources…" rows={2}
...
function StudioPanel({
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-[rgba(5,7,16,0.88)]">
      <div className="border-b border-white/10 px-4 py-3 flex-none">
        <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
...
// ─── Sub-component: NotebookView ─────────────────────────────────────────────
// Handles the Sources | Chat | Studio layout — stacked on mobile, side-by-side on md+
...
function NotebookView({
  ...
  const [mobilePanel, setMobilePanel] = useState<NotebookPanel>("chat");

  const MOBILE_TABS: [NotebookPanel, string][] = [
    ["sources", "Sources"],
    ["chat", "Chat"],
    ["studio", "Create"],
  ];
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      {/* Mobile sub-nav — hidden on md+ */}
      <div className="flex border-b border-white/10 md:hidden flex-none">
        {MOBILE_TABS.map(([panel, label]) => (
          <button key={panel} onClick={() => setMobilePanel(panel)}
            className={`flex-1 py-2.5 text-sm font-semibold transition ${
              mobilePanel === panel
                ? "border-b-2 border-[oklch(0.72_0.18_200)] text-[oklch(0.83_0.15_200)]"
      ...
      <div className="hidden md:flex flex-1 min-h-0 overflow-hidden">
        <div className="w-64 flex-none flex flex-col border-r border-white/10 overflow-hidden">
          <SourcesPanel sources={sources} activeSourceId={activeSourceId}
            onSelectSource={onSelectSource} onAddSources={onAddSources} isLoading={isLoading} />
        </div>
        <ChatPanel history={chatHistory} onSend={onSendChat} isSending={isSending} />
        <div className="w-80 flex-none flex flex-col border-l border-white/10 overflow-hidden">
          <StudioPanel studio={studio} hasProject={!!activeProjectId}
            onGenerate={onGenerate} onGenerateAudio={onGenerateAudio}
            latestAudio={latestAudio} isGeneratingAudio={isGeneratingAudio} />
        </div>
      </div>

      {/* Mobile: single active panel */}
      <div className="flex flex-1 min-h-0 overflow-hidden md:hidden">
        {mobilePanel === "sources" && (
          <SourcesPanel sources={sources} activeSourceId={activeSourceId}
            onSelectSource={(id) => { onSelectSource(id); setMobilePanel("chat"); }}
            onAddSources={onAddSources} isLoading={isLoading} />
        )}
        {mobilePanel === "chat" && (
          <ChatPanel history={chatHistory} onSend={onSendChat} isSending={isSending} />
        )}
...
function KnowledgeTreeView({
  ...
  useEffect(() => {
    ...
    const sim = forceSimulation<KTNode>(nodes)
      ...
      .force("collide", forceCollide<KTNode>().radius((d: KTNode) => nodeRadius(d, rId) + 6).iterations(2))
      .alphaDecay(0.07)
      .velocityDecay(0.35)
  ...
  function handleTouchMove(e: React.TouchEvent) {
    ...
    e.preventDefault();
    const t = e.touches[0];
    panRef.current = { x: t.clientX - touchStartRef.current.x, y: t.clientY - touchStartRef.current.y };
    setPan({ ...panRef.current });
  }
  function handleTouchEnd() { touchStartRef.current = null; }
  ...
  function nodeRadius(n: KTNode, rId: number | null): number {
    if (n.id === rId) return 24;
    const base = n.state === "explored" ? 12 : 8;
    return base + Math.min(n.neighbors.size * 1.4 + n.seenCount * 1.0, 18);
  }
  ...
  function createNode(topic: string, opts: { depth?: number; parentX?: number; parentY?: number; idx?: number; total?: number } = {}): KTNode {
    ...
    return {
      id: _ktNextId++, topic, canonicalKey: normTopic(topic),
      state: "unexplored", collapsed: false, summary: "", subtopics: [],
      depth: opts.depth ?? 0, seenCount: 1,
      discoveredFrom: new Set(), neighbors: new Set(),
  ...
  async function startExplore() {
    ...
    if (!s) return;
    if (simRef.current) simRef.current.stop();
    nodesRef.current = []; linksRef.current = []; _ktNextId = 0;
    setSelectedId(null); setRootId(null); setShowWiki(false);
    ...
    while (queue.length) {
      ...
      for (const l of links) {
        if (l.sourceId === pid && !visited.has(l.targetId)) {
          visited.add(l.targetId); hidden.add(l.targetId); queue.push(l.targetId);
        }
      }
    }
  }
  ...
  // Wiki panel content (shared between mobile sheet and desktop sidebar)
  const WikiContent = () => (
    selectedNode ? (
      <>
        <div className="border-b border-white/10 p-4 flex-none bg-[rgba(9,11,20,0.95)]">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-base font-bold text-foreground leading-tight">{selectedNode.topic}</h2>
              <div className="mt-1.5 flex flex-wrap gap-1.5 text-xs text-muted-foreground">
                ...
                <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5">{selectedNode.neighbors.size} links</span>
            ...
            <div className="flex flex-col gap-1.5 flex-none">
              {selectedNode.summary && (
                <button onClick={() => onAddAsSource(selectedNode.topic, selectedNode.summary)}
                  className="rounded-xl bg-[oklch(0.72_0.18_140_/_0.18)] px-2.5 py-1.5 text-xs font-semibold text-[oklch(0.84_0.14_140)] transition hover:bg-[oklch(0.72_0.18_140_/_0.26)]">
                  + Source
                </button>
              )}
              {selectedNode.state === "explored" && (
                <button onClick={() => { selectedNode.collapsed = !selectedNode.collapsed; setTick(t => t + 1); }}
                  className="flex items-center gap-1 rounded-xl border border-white/10 bg-white/5 px-2.5 py-1.5 text-xs text-muted-foreground transition hover:text-foreground">
                  {selectedNode.collapsed ? <Maximize2 size={11} /> : <Minimize2 size={11} />}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {selectedNode.state === "loading" ? (
            ...
            <p className="text-sm text-muted-foreground">Click this node in the graph to explore it.</p>
          ) : (
            <>
              <div className="prose prose-invert prose-sm max-w-none">
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-3 p-6 text-center text-muted-foreground">
        ...
        <p className="text-xs">Enter a seed topic and tap Explore. Node size = importance. Tap a node twice to collapse its branch.</p>
      </div>
    )
  );

  const graphControls = (
    <div className="border-b border-white/10 p-3 space-y-2.5 bg-[rgba(9,11,20,0.95)] flex-none">
      <div className="flex items-center justify-between gap-2">
        <div className="flex gap-2 flex-1">
          <input value={seed} onChange={(e) => setSeed(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && startExplore()}
            placeholder="Seed topic…"
            className="flex-1 min-w-0 rounded-xl border border-white/10 bg-[rgba(6,8,18,0.7)] px-3 py-2 text-sm text-foreground outline-none transition focus:border-[oklch(0.72_0.18_200_/_0.45)]" />
          <button onClick={startExplore} disabled={!seed.trim() || expandMutation.isPending}
            ...
            {expandMutation.isPending ? <Loader2 size={14} className="animate-spin" /> : "Explore"}
          </button>
        </div>
        <span className="hidden sm:block rounded-full border border-[oklch(0.72_0.18_200_/_0.3)] bg-[oklch(0.72_0.18_200_/_0.1)] px-2.5 py-1 text-xs font-bold text-[oklch(0.83_0.15_200)] flex-none">
          {status}
        </span>
      </div>
      <div className="flex gap-2">
        {[
          ...
          <div key={label} className="flex-1 rounded-xl border border-white/10 bg-white/5 px-2 py-1.5 text-center">
            <p className="text-xs uppercase tracking-widest text-muted-foreground leading-none mb-0.5">{label}</p>
  ...
  const graphSvg = (
    <div className="relative flex-1 overflow-hidden bg-[radial-gradient(circle_at_center,oklch(0.18_0.04_220_/_0.4),transparent_60%),rgba(5,7,16,0.97)]">
      <svg ref={svgRef} className="h-full w-full touch-none"
        ...
        onMouseDown={handleSvgMouseDown} onWheel={handleWheel}
        onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}
        onClick={(e) => { if ((e.target as SVGElement).closest("[data-node]")) return; setSelectedId(null); }}>
        <defs>
        ...
        <g transform={`translate(${pan.x},${pan.y}) scale(${scale})`}>
          {visibleLinks.map((link) => {
            ...
            const stroke = link.type === "convergent" ? "rgba(139,92,246,0.6)"
              : link.type === "seed" ? "rgba(37,99,235,0.5)" : "rgba(148,163,184,0.3)";
            return <line key={link.id} x1={src.x} y1={src.y} x2={tgt.x} y2={tgt.y}
              stroke={stroke} strokeWidth={link.type === "convergent" ? 2 : 1.2} />;
          })}
          {visibleNodes.map((node) => {
            const r = nodeRadius(node, rootId);
            ...
            return (
              <g key={node.id} data-node="1" style={{ cursor: "pointer" }}
                ...
                {isSelected && <circle cx={node.x} cy={node.y} r={r + 12} fill="none"
                ...
                {node.collapsed && (
                  <text x={node.x} y={node.y + r + 14} textAnchor="middle"
                    fill="rgba(167,243,208,0.9)" fontSize={10} fontFamily="system-ui, sans-serif"
                    fontWeight={700} stroke="rgba(15,23,42,0.7)" strokeWidth={3} paintOrder="stroke" pointerEvents="none">
                    {labelLines[0]}
                  </text>
                )}
              </g>
      ...
      {/* Legend */}
      <div className="absolute bottom-2 left-2 flex flex-wrap gap-2 rounded-xl border border-white/10 bg-[rgba(9,11,20,0.75)] px-2.5 py-1.5 backdrop-blur text-xs text-muted-foreground">
        {[
      ...
      {/* Mobile: wiki toggle button */}
      {selectedNode && (
        <button onClick={() => setShowWiki(v => !v)}
          className="absolute bottom-2 right-2 md:hidden rounded-xl border border-white/15 bg-[rgba(9,11,20,0.88)] px-3 py-1.5 text-xs font-semibold text-foreground backdrop-blur">
          {showWiki ? "Hide Info" : "Show Info"}
        </button>
      )}
    </div>
  );
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col overflow-hidden">
      ...
      {/* Desktop: graph + wiki side by side */}
      <div className="hidden md:flex flex-1 min-h-0 overflow-hidden">
        <div className="flex flex-col border-r border-white/10 flex-1 min-w-0">
          {graphSvg}
        </div>
        <div className="flex flex-col overflow-hidden bg-[rgba(9,11,20,0.92)] flex-none" style={{ width: 340 }}>
          <WikiContent />
        </div>
      </div>

      {/* Mobile: graph then wiki sheet below */}
      <div className="flex md:hidden flex-1 min-h-0 flex-col overflow-hidden">
        <div className="flex flex-col" style={{ flex: showWiki && selectedNode ? "0 0 45%" : "1 1 auto" }}>
          {graphSvg}
        </div>
        {showWiki && selectedNode && (
          <div className="flex flex-col border-t border-white/15 bg-[rgba(9,11,20,0.96)] overflow-hidden flex-none" style={{ height: "55%" }}>
            <WikiContent />
          </div>
        )}
      </div>
...
function ChartsView({ cookieId }: { cookieId: string }) {
  ...
  async function analyze() {
    ...
    try {
      ...
      const base = subs.map((label, i) => ({ label, value: Math.round(95 - i * 6), category: "primary" }));
      setItems(base);
      for (let i = 0; i < Math.min(4, subs.length); i++) {
        try {
          ...
          const secondary = (sub.subtopics ?? []).slice(0, 3).map((label, j) => ({
            label, value: Math.round(48 - j * 4), category: subs[i],
          }));
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-y-auto p-4 sm:p-6 gap-4 sm:gap-6">
      <div>
        <h2 className="text-xl font-bold text-foreground mb-1">Topic Analysis</h2>
        <p className="text-sm text-muted-foreground mb-4">Visualize how a topic branches into subtopics by importance.</p>
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <button onClick={analyze} disabled={loading || !topic.trim()}
            ...
            {loading ? <Loader2 size={14} className="animate-spin mx-auto" /> : "Analyze"}
          </button>
        </div>
      </div>
      ...
      {items.length > 0 && (
        <>
            ...
            {VIEW_TABS.map(([v, label]) => (
              <button key={v} onClick={() => setView(v)}
                className={`rounded-xl px-3 py-1.5 text-sm font-semibold transition ${
                  view === v ? "bg-[oklch(0.72_0.18_200_/_0.18)] text-[oklch(0.83_0.15_200)]" : "bg-white/5 text-muted-foreground hover:text-foreground"
                }`}>
                {label}
              </button>
            ))}
            {loading && <Loader2 size={14} className="self-center animate-spin text-muted-foreground" />}
          </div>
              ...
              {primaryItems.length > 0 && (
                <div>
                  <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-2">
                    Primary — <span className="normal-case font-normal">{topic}</span>
                  </p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-1.5">
                    {secondaryItems.map((item, i) => (
                      <div key={i} className="rounded-xl p-2 text-center cursor-default"
                        style={{ background: importanceColor(item.value) }}>
            ...
            <div className="space-y-1.5">
              {[...items].sort((a, b) => b.value - a.value).map((item, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground w-28 sm:w-36 truncate flex-none text-right leading-none">{item.label}</span>
          ...
          {view === "bubbles" && (
            <div className="flex flex-wrap gap-2 sm:gap-3 items-end justify-center py-4 min-h-[240px]">
              {[...items].sort((a, b) => b.value - a.value).map((item, i) => {
                const size = 32 + (item.value / maxValue) * 90;
                return (
                  <div key={i} title={item.label}
                    ...
                    <span className="text-white/90 font-semibold text-center px-1 leading-tight overflow-hidden"
                      style={{ fontSize: Math.max(8, size / 9), maxWidth: size - 8 }}>
      ...
      {!loading && items.length === 0 && (
        <div className="flex flex-col items-center justify-center flex-1 text-center text-muted-foreground gap-3 min-h-[240px]">
          <BarChart2 size={44} className="opacity-20" />
          <h3 className="text-base font-semibold text-foreground">Visualize any topic</h3>
          <p className="text-sm max-w-xs">Enter a topic above to generate heat maps, bar charts, and bubble diagrams.</p>
        </div>
      )}
    </div>
  );
...
interface HoleContent { topic: string; summary: string; subtopics: string[]; }
...
function RabbitHolesView({ cookieId }: { cookieId: string }) {
  ...
  async function enterHole(id: RabbitHoleId) {
    ...
    if (!hole) return;
    setActiveId(id); setContentStack([]); setIsLoading(true);
    try {
  ...
  function goBack() {
    if (contentStack.length > 1) setContentStack((prev) => prev.slice(0, -1));
    else { setActiveId(null); setContentStack([]); }
  }
  ...
  if (activeHole) {
    return (
      <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
        {/* Header */}
        <div className="border-b border-white/10 bg-[rgba(9,11,20,0.95)] px-4 sm:px-6 py-3 flex-none">
          <div className="flex items-center gap-2 mb-2 flex-wrap text-xs text-muted-foreground">
            <button onClick={goBack} className="hover:text-foreground transition flex items-center gap-1">
              ← {contentStack.length > 1 ? "Back" : "All Holes"}
            </button>
            {contentStack.length > 1 && contentStack.map((c, i) => (
              <span key={i} className="flex items-center gap-1">
                <span>›</span>
                <button onClick={() => setContentStack((prev) => prev.slice(0, i + 1))}
                  className={`hover:text-foreground transition truncate max-w-[100px] ${i === contentStack.length - 1 ? "text-foreground font-semibold" : ""}`}>
                  {c.topic.split(" ").slice(0, 2).join(" ")}
                </button>
              </span>
            ))}
          </div>
          <div className="flex items-start gap-3">
            <span className="text-3xl flex-none">{activeHole.emoji}</span>
            <div className="min-w-0">
              ...
              {depth === 0 && <p className="mt-1 text-xs sm:text-sm text-muted-foreground leading-relaxed">{activeHole.hook}</p>}
              <div className="mt-1.5 flex gap-2 flex-wrap">
                <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-xs text-muted-foreground capitalize">{activeHole.domain}</span>
                {depth > 0 && (
                  <span className="rounded-full border border-[oklch(0.72_0.18_200_/_0.3)] bg-[oklch(0.72_0.18_200_/_0.1)] px-2.5 py-0.5 text-xs text-[oklch(0.83_0.15_200)]">
                    Depth {depth}
                  </span>
                )}
              </div>
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 min-w-0">
            {isLoading && (
              <div className="flex items-center gap-2 text-muted-foreground">
                <Loader2 size={16} className="animate-spin" /><span className="text-sm">Going deeper…</span>
              </div>
            )}
            {currentContent && !isLoading && (
              <>
                ...
                {currentContent.subtopics.length > 0 && (
                  <div>
                    <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground">Follow the Thread</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {currentContent.subtopics.slice(0, 6).map((subtopic, i) => (
                        <button key={i} onClick={() => goDeeper(subtopic)}
                          ...
                          <span className="block text-sm font-semibold text-foreground group-hover:text-[oklch(0.83_0.15_200)] transition mb-0.5">{subtopic}</span>
                          <span className="text-xs text-muted-foreground">Tap to go deeper →</span>
                        </button>
                      ))}
                    </div>
                  </div>
                )}
                <button onClick={() => currentContent.subtopics[0] && goDeeper(currentContent.subtopics[0])}
                  disabled={isLoading || currentContent.subtopics.length === 0}
                  className="flex items-center gap-2 rounded-2xl border border-[oklch(0.72_0.18_200_/_0.3)] bg-[oklch(0.72_0.18_200_/_0.08)] px-5 py-3 text-sm font-semibold text-[oklch(0.83_0.15_200)] transition hover:bg-[oklch(0.72_0.18_200_/_0.16)] disabled:opacity-40">
                  <Zap size={14} /> Go Even Deeper{depth > 0 ? ` (Level ${depth + 1})` : ""}
                </button>
              </>
            )}

            {/* Mobile: other holes inline */}
            <div className="md:hidden">
              <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground">Other Rabbit Holes</p>
              <div className="flex flex-wrap gap-2">
                {RABBIT_HOLES.filter((h) => h.id !== activeId).slice(0, 6).map((h) => (
                  <button key={h.id} onClick={() => enterHole(h.id)}
                    className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-muted-foreground transition hover:text-foreground">
          ...
          {/* Desktop sidebar */}
          <div className="hidden md:flex flex-col w-64 flex-none border-l border-white/10 bg-[rgba(9,11,20,0.92)] overflow-y-auto p-3">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-2 px-1">Other Holes</p>
            {RABBIT_HOLES.filter((h) => h.id !== activeId).map((h) => (
              <button key={h.id} onClick={() => enterHole(h.id)}
                className="flex w-full items-start gap-2 rounded-xl p-2.5 text-left transition hover:bg-white/5 group">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground capitalize">{h.domain}</p>
                </div>
              </button>
            ))}
          </div>
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-y-auto p-4 sm:p-6 gap-4 sm:gap-6">
      <div>
        <h2 className="text-xl sm:text-2xl font-bold text-foreground mb-1">🐇 Rabbit Holes</h2>
        <p className="text-sm text-muted-foreground max-w-2xl">
          Twelve curated deep-dives into the most fascinating corners of human knowledge. Choose a hole and go as deep as you dare.
        </p>
      </div>
      <div className="flex gap-2 flex-wrap">
        {domains.map((d) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        {filtered.map((hole) => (
          <button key={hole.id} onClick={() => enterHole(hole.id)}
            className="glass rounded-2xl p-4 sm:p-5 border border-white/10 text-left transition hover:border-[oklch(0.72_0.18_200_/_0.3)] hover:bg-[oklch(0.72_0.18_200_/_0.05)] group">
            <div className="flex items-start gap-3 mb-2">
              <span className="text-2xl sm:text-3xl flex-none">{hole.emoji}</span>
              <div className="min-w-0">
                <h3 className="text-sm font-bold text-foreground group-hover:text-[oklch(0.83_0.15_200)] transition leading-snug">{hole.title}</h3>
                <span className="text-xs text-muted-foreground capitalize">{hole.domain}</span>
              </div>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed line-clamp-3 mb-2">{hole.teaser}</p>
...
function DiscoverModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 p-0 sm:p-4 backdrop-blur-sm">
      <div className="flex w-full sm:max-w-2xl flex-col rounded-t-3xl sm:rounded-3xl border border-white/10 bg-[rgba(9,11,20,0.97)] shadow-2xl" style={{ maxHeight: "90vh" }}>
        ...
        <div className="border-b border-white/10 p-4 space-y-2.5 flex-none">
          ...
          <div className="flex gap-2">
            <select value={count} onChange={(e) => setCount(Number(e.target.value))}
              ...
              {[5, 10, 15, 20].map((n) => <option key={n} value={n}>{n} sources</option>)}
            </select>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          {!isSearching && results.length === 0 && (
            <div className="py-8 text-center text-sm text-muted-foreground">Results will appear after searching.</div>
          )}
          {results.map((r, i) => (
            <label key={i} className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-3 transition ${selected.has(r.url) ? "border-[oklch(0.72_0.18_200_/_0.4)] bg-[oklch(0.72_0.18_200_/_0.08)]" : "border-white/10 hover:border-white/20"}`}>
              <input type="checkbox" checked={selected.has(r.url)}
                onChange={() => setSelected((prev) => { const next = new Set(prev); if (next.has(r.url)) next.delete(r.url); else next.add(r.url); return next; })}
        ...
        {results.length > 0 && (
          <div className="flex items-center justify-between border-t border-white/10 px-5 py-4 flex-none">
            <button onClick={toggleAll} className="text-xs text-muted-foreground hover:text-foreground transition">
              {selected.size === results.length ? "Deselect All" : "Select All"}
            </button>
            <div className="flex gap-2">
              <button onClick={onClose} className="rounded-xl border border-white/10 px-4 py-2 text-sm text-muted-foreground transition hover:text-foreground">Cancel</button>
              <button onClick={() => { onImport(topic.trim(), Array.from(selected)); onClose(); }}
                disabled={selected.size === 0}
                className="rounded-xl bg-[oklch(0.72_0.18_140_/_0.2)] px-4 py-2 text-sm font-semibold text-[oklch(0.84_0.14_140)] transition hover:bg-[oklch(0.72_0.18_140_/_0.3)] disabled:opacity-40">
...
export default function ResearchWorkbench() {
  ...
  const projectsQuery = trpc.research.listProjects.useQuery(
    { cookieId: cookieId ?? "" }, { enabled: !!cookieId });
  ...
  const chatMutation = trpc.research.ragChat.useMutation({
    onSuccess: (data) => {
      setChatHistory((prev) => [...prev.slice(0, -1), { role: "assistant", content: data.answer, citations: data.citations }]);
      addXP(3);
    },
    onError: (err) => {
      setChatHistory((prev) => [...prev.slice(0, -1), { role: "assistant", content: `Error: ${err.message}` }]);
    },
  });
  ...
  const generateContentMutation = trpc.research.generateContent.useMutation({
    ...
    onError: (err) => { setStudio({ mode: "idle" }); toast.error(err.message); },
  });
  ...
  useEffect(() => {
    setChatHistory([]); setActiveSourceId(null); setStudio({ mode: "idle" });
  }, [activeProjectId]);
      ...
      <div className="flex flex-col" style={{ height: "calc(100vh - 4rem)", paddingTop: "4rem" }}>
        ...
        {activeTab === "notebook" ? (
          activeProjectId && workspace ? (
            <NotebookView
              sources={sources} activeSourceId={activeSourceId}
              onSelectSource={setActiveSourceId} onAddSources={() => setShowDiscover(true)}
              isLoading={workspaceQuery.isLoading} chatHistory={chatHistory}
              onSendChat={handleSendChat} isSending={chatMutation.isPending}
              studio={studio} onGenerate={handleGenerate}
              onGenerateAudio={handleGenerateAudio} latestAudio={latestAudio}
              isGeneratingAudio={audioMutation.isPending} activeProjectId={activeProjectId}
            />
          ) : (
            <div className="flex flex-1 flex-col items-center justify-center gap-4 text-center text-muted-foreground p-6">
              {projectsQuery.isLoading || ingestMutation.isPending ? (
                ...
                <>
                  ...
                  <p className="max-w-xs text-sm">Discover sources around a topic to create your first notebook.</p>
                  <button onClick={() => setShowDiscover(true)}

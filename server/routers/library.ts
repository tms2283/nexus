...
export const libraryRouter = router({
  list: publicProcedure
    ...
    .query(async ({ input }) =>
      getLibraryResources(input.category, input.search)
    ),
  ...
  contribute: visitorProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const existing = await db
        ...
        .where(eq(libraryResources.url, input.url))
        .limit(1);
      if (existing.length > 0)
        throw new TRPCError({
          code: "CONFLICT",
          message: "This resource is already in the Library.",
        });
      await db
        ...
        .values({
          ...
          type: input.type,
          featured: false,
          contributedBy: cookieId,
        });
      const xpResult = await addXP(cookieId, 50);
      return {
        success: true,
        message: "Resource added to the Library! +50 XP",
        ...xpResult,
      };
    }),
  ...
  listCommunity: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!db)
        return {
          items: [] as (typeof libraryResources.$inferSelect)[],
          total: 0,
        };
  ...
  getReadingList: visitorProcedure.query(async ({ ctx }) => {
    const cookieId = getVisitorId(ctx);
    const db = await getDb();
    if (!db) return { items: [] };
    const items = await db
      .select()
      .from(readingList)
      .where(eq(readingList.cookieId, cookieId))
      .orderBy(desc(readingList.addedAt));
    return { items };
  }),

  addToReadingList: visitorProcedure
    .input(
      z.object({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.resourceId) {
        const existing = await db
          ...
          .where(
            and(
              eq(readingList.cookieId, cookieId),
              eq(readingList.resourceId, input.resourceId)
            )
          )
          .limit(1);
  ...
  markRead: visitorProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db
        ...
        .where(
          and(eq(readingList.id, input.id), eq(readingList.cookieId, cookieId))
        );
      await addXP(cookieId, 5);
      return { success: true };
    }),

  moveToStatus: visitorProcedure
    .input(
      z.object({
        id: z.number(),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (isRead) await addXP(cookieId, 5);
      return { success: true };
    }),

  removeFromReadingList: visitorProcedure
    .input(z.object({ id: z.number() }))
    .mutation(async ({ input, ctx }) => {
      const cookieId = getVisitorId(ctx);
      const db = await getDb();

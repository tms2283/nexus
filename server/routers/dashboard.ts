  ...
  getTopUsers: publicProcedure
    .input(
      z.object({
        ...
        metric: z.enum(["xp", "streak", "visitCount"]).default("xp"),
      })
    )
    .query(async ({ input }) => {
      ...
      const col =
        input.metric === "xp"
          ? visitorProfiles.xp
          : input.metric === "streak"
            ? visitorProfiles.streak
            : visitorProfiles.visitCount;
      const rows = await db
        .select({
        ...
        .orderBy(descOrder(col))
        .limit(input.limit);
      return {
        users: rows.map((r, i) => ({
          ...r,
          rank: i + 1,
          displayName: `Explorer #${r.cookieId.slice(-6).toUpperCase()}`,
        })),
      };
    }),

  getMyRank: visitorProcedure
    .input(
      z.object({ metric: z.enum(["xp", "streak", "visitCount"]).default("xp") })
    )
...
export const dashboardRouter = router({
  getActivity: visitorProcedure.query(async ({ ctx }) => {
    const cookieId = ctx.visitorCookieId!;
    const db = await getDb();
    ...
    const [researchCounts, testCounts, iqCounts, reviewCounts] =
      await Promise.all([
        db
          .select({
          ...
          .where(sql`cookieId = ${cookieId} AND createdAt >= ${cutoff}`)
          .groupBy(sql`DATE(createdAt)`),
        db
          .select({
            date: sql<string>`DATE(completedAt)`,
            count: sql<number>`COUNT(*)`,
          })
          .from(tr)
          .where(sql`cookieId = ${cookieId} AND completedAt >= ${cutoff}`)
        ...
        db
          .select({
            date: sql<string>`DATE(fr.reviewedAt)`,
            count: sql<number>`COUNT(*)`,
          })
          .from(flashcardReviews.as("fr"))
          .innerJoin(
            flashcardDecks,
            eq(flashcardReviews.deckId, flashcardDecks.id)
          )
          .where(sql`fd.cookieId = ${cookieId} AND fr.reviewedAt >= ${cutoff}`)
    ...
    for (const rows of [researchCounts, testCounts, iqCounts, reviewCounts]) {
      for (const row of rows) {
        if (row.date)
          dayMap[row.date] = (dayMap[row.date] ?? 0) + Number(row.count);
      }
    }
    ...
    return {
      ...
      totalActions,
      avgPerDay:
        totalActions > 0 ? Math.round((totalActions / 84) * 10) / 10 : 0,
    };
  }),

  getInsight: visitorProcedure.query(async ({ ctx }) => {
    const cookieId = ctx.visitorCookieId!;
    const db = await getDb();
    if (!db) return { insight: null };
    const { testResults: tr, iqResults: iq } =
      await import("../../drizzle/schema");
    const [tests, iqs] = await Promise.all([
      db
        .select()
        .from(tr)
        .where(eq(tr.cookieId, cookieId))
        .orderBy(descOrder(tr.completedAt))
        .limit(20),
      db
        .select()
        .from(iq)
        .where(eq(iq.cookieId, cookieId))
        .orderBy(descOrder(iq.completedAt))
        .limit(1),
    ]);
    ...
    const strongest = subjectAvgs[0],
      weakest = subjectAvgs[subjectAvgs.length - 1];
    ...
    if (latestIQ) {
      const tier =
        latestIQ.iqScore >= 130
          ? "exceptional"
          : latestIQ.iqScore >= 115
            ? "above average"
            : latestIQ.iqScore >= 100
              ? "average"
              : "developing";
    ...
    if (tests.length >= 3) {
      const recent = tests
        .slice(0, 3)
        .map(t => Math.round((t.score / t.totalQuestions) * 100));
      insightText += `Recent trend: **${recent[0] > recent[2] ? "improving" : recent[0] < recent[2] ? "declining" : "stable"}**.`;
    }

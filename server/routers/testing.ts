...
export const testingRouter = router({
  saveResult: visitorProcedure
    .input(z.object({ testId: z.string().min(1), score: z.number(), totalQuestions: z.number(), answers: z.record(z.string(), z.number()), timeTakenSeconds: z.number().optional() }))
    .mutation(async ({ ctx, input }) => {
      const cookieId = ctx.visitorCookieId!;
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database unavailable" });
      await db.insert(testResults).values({ cookieId, testId: input.testId, score: input.score, totalQuestions: input.totalQuestions, answers: input.answers, timeTakenSeconds: input.timeTakenSeconds ?? null });
      const pct = input.score / input.totalQuestions;
      const xp = pct >= 0.9 ? 100 : pct >= 0.7 ? 60 : pct >= 0.5 ? 30 : 15;
      const xpResult = await addXP(cookieId, xp);
      return { success: true, xpAwarded: xp, ...xpResult };
    }),

  saveIQResult: visitorProcedure
    .input(z.object({ iqScore: z.number().min(40).max(200), percentile: z.number(), rawScore: z.number(), categoryScores: z.record(z.string(), z.number()), timeTakenSeconds: z.number().optional() }))
    .mutation(async ({ ctx, input }) => {
      const cookieId = ctx.visitorCookieId!;
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database unavailable" });
      await db.insert(iqResults).values({ cookieId, iqScore: input.iqScore, percentile: input.percentile, rawScore: input.rawScore, categoryScores: input.categoryScores, timeTakenSeconds: input.timeTakenSeconds ?? null });
      const xpResult = await addXP(cookieId, 75);
      return { success: true, xpAwarded: 75, ...xpResult };
    }),

  getResults: visitorProcedure
    .query(async ({ ctx }) => {
      const cookieId = ctx.visitorCookieId!;
      const db = await getDb();
      if (!db) return { testResults: [], iqResults: [] };
      const [tests, iqs] = await Promise.all([
        db.select().from(testResults).where(eq(testResults.cookieId, cookieId)).orderBy(desc(testResults.completedAt)).limit(50),
        db.select().from(iqResults).where(eq(iqResults.cookieId, cookieId)).orderBy(desc(iqResults.completedAt)).limit(10),
      ]);
      return { testResults: tests, iqResults: iqs };
    }),

  getScoreHistory: visitorProcedure
    .query(async ({ ctx }) => {
      const cookieId = ctx.visitorCookieId!;
      const db = await getDb();
      ...
      const [tests, iqs] = await Promise.all([
        db.select({ id: testResults.id, testId: testResults.testId, score: testResults.score, totalQuestions: testResults.totalQuestions, timeTakenSeconds: testResults.timeTakenSeconds, completedAt: testResults.completedAt }).from(testResults).where(eq(testResults.cookieId, cookieId)).orderBy(testResults.completedAt),
        db.select({ id: iqResults.id, iqScore: iqResults.iqScore, percentile: iqResults.percentile, rawScore: iqResults.rawScore, categoryScores: iqResults.categoryScores, timeTakenSeconds: iqResults.timeTakenSeconds, completedAt: iqResults.completedAt }).from(iqResults).where(eq(iqResults.cookieId, cookieId)).orderBy(iqResults.completedAt),
      ]);
  ...
  generateAdaptiveQuestions: visitorProcedure
    .input(z.object({ subject: z.string(), difficulty: z.enum(["beginner", "intermediate", "advanced"]), count: z.number().min(3).max(15).default(10) }))
    .mutation(async ({ ctx, input }) => {
      const cookieId = ctx.visitorCookieId!;
      ...
      const raw = await callAI(cookieId, prompt, "You are an expert educator. Return only valid JSON.", 2000);
      try {
    ...
    .query(async ({ input }) => {
      ...
      const [tests, iqs] = await Promise.all([
        ...
        db
          ...
          .orderBy(desc(iqResults.completedAt))
          .limit(10),
      ]);
      return { testResults: tests, iqResults: iqs };
    }),
    ...
    .query(async ({ input }) => {
      ...
      for (const t of tests) {
        ...
        subjects[t.testId].push({
          attempt: subjects[t.testId].length + 1,
          score: t.score,

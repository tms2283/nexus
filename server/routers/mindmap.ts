...
export const mindmapRouter = router({
  list: publicProcedure.input(z.object({ cookieId: z.string() })).query(async ({ input }) => getMindMaps(input.cookieId)),
  get: publicProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => getMindMapById(input.id)),
